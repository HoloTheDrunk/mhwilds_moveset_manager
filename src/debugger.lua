...
local function mutate_bindings(_, name, value)
  ...
  do
    local i = 1; repeat
      ...
      if name == var then
        dbg_writeln((dbg.COLOR_YELLOW) ..
        "debugger.lua" .. (dbg.GREEN_CARET) .. "Set local variable " .. (dbg.COLOR_BLUE) .. name .. (dbg.COLOR_RESET))
  ...
  do
    local i = 1; repeat
      ...
      if name == var then
        dbg_writeln((dbg.COLOR_YELLOW) ..
        "debugger.lua" .. (dbg.GREEN_CARET) .. "Set upvalue " .. (dbg.COLOR_BLUE) .. name .. (dbg.COLOR_RESET))
  ...
  -- Set a global.
  dbg_writeln((dbg.COLOR_YELLOW) ..
  "debugger.lua" .. (dbg.GREEN_CARET) .. "Set global variable " .. (dbg.COLOR_BLUE) .. name .. (dbg.COLOR_RESET))
  _G[name] = value
end
...
local function cmd_trace()
  ...
  local i = 0; while true do
    ...
    dbg_writeln((dbg.COLOR_GRAY) .. "% 4d" .. (dbg.COLOR_RESET) .. tab_or_caret .. "%s", i,
...
local function cmd_help()
  dbg.write(""
    ...
    .. (dbg.COLOR_BLUE) ..
    "  s" .. (dbg.COLOR_YELLOW) .. "(tep)" .. (dbg.GREEN_CARET) .. "step forward by one line (into functions)\n"
    ..
    (dbg.COLOR_BLUE) ..
    "  n" .. (dbg.COLOR_YELLOW) .. "(ext)" .. (dbg.GREEN_CARET) .. "step forward by one line (skipping over functions)\n"
    ..
    (dbg.COLOR_BLUE) ..
    "  f" .. (dbg.COLOR_YELLOW) .. "(inish)" .. (dbg.GREEN_CARET) .. "step forward until exiting the current function\n"
    .. (dbg.COLOR_BLUE) .. "  u" .. (dbg.COLOR_YELLOW) .. "(p)" .. (dbg.GREEN_CARET) ..
    "move up the stack by one frame\n"
    .. (dbg.COLOR_BLUE) .. "  d" .. (dbg.COLOR_YELLOW) ..
    "(own)" .. (dbg.GREEN_CARET) .. "move down the stack by one frame\n"
    ..
    (dbg.COLOR_BLUE) ..
    "  i" ..
    (dbg.COLOR_YELLOW) .. "(nspect) " ..
    (dbg.COLOR_BLUE) .. "[index]" .. (dbg.GREEN_CARET) .. "move to a specific stack frame\n"
    ..
    (dbg.COLOR_BLUE) ..
    "  w" ..
    (dbg.COLOR_YELLOW) ..
    "(here) " .. (dbg.COLOR_BLUE) .. "[line count]" .. (dbg.GREEN_CARET) .. "print source code around the current line\n"
    ..
    (dbg.COLOR_BLUE) ..
    "  e" .. (dbg.COLOR_YELLOW) ..
    "(val) " .. (dbg.COLOR_BLUE) .. "[statement]" .. (dbg.GREEN_CARET) .. "execute the statement\n"
    ..
    (dbg.COLOR_BLUE) ..
    "  p" ..
    (dbg.COLOR_YELLOW) ..
    "(rint) " .. (dbg.COLOR_BLUE) .. "[expression]" .. (dbg.GREEN_CARET) ..
    "execute the expression and print the result\n"
    .. (dbg.COLOR_BLUE) .. "  t" .. (dbg.COLOR_YELLOW) .. "(race)" .. (dbg.GREEN_CARET) .. "print the stack trace\n"
    ..
    (dbg.COLOR_BLUE) ..
    "  l" .. (dbg.COLOR_YELLOW) .. "(ocals)" .. (dbg.GREEN_CARET) ..
...
function dbg.msgh(...)
  if debug.getinfo(2) then
    ...
    dbg_writeln((dbg.COLOR_RED) ..
    "debugger.lua: " .. (dbg.COLOR_RESET) ..
